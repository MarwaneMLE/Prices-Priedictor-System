...
model = Model(
    ...
    version=None,
    license="Apache 2.0",
    description="Houses price prediction model."
)

@step(enable_cache=False, experiment_tracker=experiment_tracker.name, model=model)
def model_bulding_step(X_train: pd.DataFrame, y_train: pd.Series) -> Annotated[Pipeline, ArtifactConfig(name="sklearn_pipline", is_model_artifact=True)]:
    ...
    categorical_transformer = Pipeline(
        [
            ("imputer", SimpleImputer(strategy="most_frequent")), # Feature scaling
            ("onehot", OneHotEncoder(handle_unknown="ignore")) # Linear regression model
        ]
    )
